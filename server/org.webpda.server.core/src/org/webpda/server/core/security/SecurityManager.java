...
public class SecurityManager {
				...
				if (clazz != null) {
					...
					if (newInstance instanceof AuthorizationProvider)
						...
						LoggerUtil.getLogger().log(Level.SEVERE, 
								"Failed to create authorization provider:" + authorizationProviderName);
				} else
					LoggerUtil.getLogger().log(Level.SEVERE,"Authorization provider class was not found: " + authorizationProviderName);
			} catch (Exception e) {
				LoggerUtil.getLogger().log(Level.SEVERE, 
						"Failed to create authorization provider:" + authorizationProviderName, e);
			} 			
		}else {
			LoggerUtil.getLogger().log(Level.WARNING, "Authorization provider is not set.");
		}
	 ...
	 */
	public static UserSecurityContext login(String userName, String passWord) throws Exception{
		String jaasName = System.getProperty(ConfigurePropertyConstants.JAAS_ENTRY);
		if(jaasName == null)
			jaasName = "Default"; 
