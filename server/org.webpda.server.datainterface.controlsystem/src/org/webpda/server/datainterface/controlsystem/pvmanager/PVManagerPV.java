...
public class PVManagerPV implements IPV {
	...
	private org.epics.pvmanager.ExceptionHandler exceptionHandler;
	...
	public PVManagerPV(final String name, final boolean readOnly,
		...
		if (exceptionHandler != null) {
			this.exceptionHandler = new org.epics.pvmanager.ExceptionHandler() {
				@Override
	...
	public void setValue(Object new_value){
		if (readOnly){
			fireExceptionOccured( new Exception("The PV was created for read only: " + getName()));
			return;
		}
		if (isFormula){
			fireExceptionOccured(new Exception("The PV is a formula which is not allowed to write: " +
					getName()));
			return;
		}
		if (pvWriter == null || pvWriter.isClosed()){
			fireExceptionOccured(new Exception("The PV is not started yet or has been closed." + getName()));
			return;
		}
	...
	public void start() throws Exception {
		if (!startFlag.getAndSet(true)) {
			...
			throw new IllegalStateException(
					"PV has already been started : " + getName());
	}
	...
	public void stop() {
		if(!startFlag.get()){
			LoggerUtil.getLogger().log(Level.WARNING, 
					"PV has already been stopped or was not started yet: " + getName());
			return;
		}		
		if (pvReader != null){
			pvReader.close();
